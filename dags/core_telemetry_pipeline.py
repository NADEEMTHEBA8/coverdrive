...
@dag(
    ...
    description="Core telemetry refresh: ingest ESPN/Cricsheet/Weather → Silver → Pandera contracts → dbt Gold marts → API readiness check.",
    schedule="0 0 * * *",
...
def coverdrive_telemetry_refresh() -> None:
    ...
    if HAS_COSMOS:
        transform_gold_marts = DbtTaskGroup(
            ...
            profile_config=ProfileConfig(profile_name="coverdrive", target_name="dev"),
            execution_config=ExecutionConfig(dbt_executable_path="/usr/local/bin/dbt"),
        )
    else:
        transform_gold_marts = BashOperator(
            task_id="dbt_build",
            bash_command=f"cd {DBT_PROJECT_DIR} && dbt deps --no-version-check && dbt build --target=dev --fail-fast",
            env={
